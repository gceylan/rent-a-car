...
<input id="roleId" type="hidden" value="${ logonUser.role.id }" />
			...
			<table class="table table-bordered table-hover toggle-circle" data-filter="#filter" data-page-size="6" data-page-previous-text="önceki" data-page-next-text="sonraki">
				<thead>
					<tr>
						...
						<th data-hide="all">Tahsis Edilen Araç</th>
						<th>Müşteri Adı</th>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('table').footable().on('click', '.row-complete', function(e) {
			event.stopPropagation();
          	var footable = $('table').data('footable');
          	var row = $(this).parents('tr:first');
			if (confirm("Tamamlansın mı?")) {
				this.click;
				$.ajax({
					type: "GET",
		            url: $(event.target).attr("href"),
	...
	function getAllRentalsJSON() {
		/*
			javascript içinde jstl çalışıyor,
			alttaki satırı silip direkt jstl karşılığını yazabiliriz
		*/
		...
		var completeUrl = contextPath + "/branch/reservations/complete/";
		...
		$.ajax({
			...
			success : function(rentals) {
				$.each(rentals, function(key, rental) {
	                ...
	                row += '<td>' + $.format.date(new Date(rental.pickupDate), "dd/MM/yyyy HH:mm:ss") + ": " + jQuery.format.prettyDate(new Date(rental.pickupDate)) + '</td>';
	                row += '<td>' + rental.pickupLocation.name + '</td>';
	                row += '<td>' + $.format.date(new Date(rental.returnDate), "dd/MM/yyyy HH:mm:ss") + ": " + jQuery.format.prettyDate(new Date(rental.returnDate)) + '</td>';
	                row += '<td>' + rental.returnLocation.name + '</td>';

	                // dikkat artı eksi 90 dk hikayesini uygula
	                if (new Date() > rental.pickupDate) {
		                if (new Date() > rental.returnDate)
			                row += '<td><span class="label label-warning">bitti ama kimse tamamlamadı</span> <a class="row-complete" href="' + completeUrl + rental.id + '">Tamamla</a></td>';
			            else
	                		row += '<td><span class="label label-info">şu anda gerçekleşiyor</span></td>';
		            }
