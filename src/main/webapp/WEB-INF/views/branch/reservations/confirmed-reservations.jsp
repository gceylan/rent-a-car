...
<script type="text/javascript">
	...
	function getAllRentalsJSON() {
		...
		*/
		...
		$.ajax({
			url : contextPath + "/branch/reservations/json-confirmed",
			dataType : 'json',
			success : function(rentals) {
				$.each(rentals, function(key, rental) {
					var dayOfMilisecond = 24 * 60 * 60 * 1000;
					var now = new Date().getTime();

					var row;

					// kiralamaya 24 saatten az kalanları farkli renkte göster
					if ((rental.pickupDate % now) < dayOfMilisecond) {
		            ...
		            if (rental.confirmed && (now < rental.pickupDate)) {
	                	row += '<td><span class="label label-primary">zamanını bekliyor</span></td>';
		            }
